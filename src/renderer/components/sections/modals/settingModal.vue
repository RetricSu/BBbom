 ...
 * @LastEditors: Retric
 * @LastEditTime: 2019-08-18 11:01:53
 -->
<template>
          ...
          <div class="partition-form">
            <form autocomplete="false">
              <div class="alert-time-picker">
                ...
                <div class="line">
                  <h2>固定价格预警</h2>
                  <input type="checkbox" id="fixed-price-check" v-model="is_fixed_open"/>
                  <label for="fixed-price-check">{{ is_fixed_open ? '已开启':'已关闭'}}</label>
                  <div v-if="is_fixed_open">
                    上涨至：<input type="number" :value="fixed_up" ref="fixed_up"> USDT
                    <br>
                    下降至：<input type="number" :value="fixed_down" ref="fixed_down"> USDT
                  </div>
                </div>
                <hr />
                <div class="line">
...
export default {
  ...
  methods: {
    loadData: function() {
      ...
      console.log(setting);
      if (!setting) {
        this.is_fixed_open = default_config.IS_FIXED_PRICE_WATCHED;
        this.fixed_up = default_config.FIXED_PRICE_UP;
        this.fixed_down = default_config.FIXED_PRICE_DOWN;
        ...
        this.is_fixed_open = setting.is_fixed_price_watched || default_config.IS_FIXED_PRICE_WATCHED;
        this.fixed_up = setting.fixed_price_up || default_config.FIXED_PRICE_UP;
        this.fixed_down = setting.fixed_price_down || default_config.FIXED_PRICE_DOWN;
        this.change_up = (setting.change_up || default_config.CHANGE_UP) * 100;
        this.change_down =
          (setting.change_down || default_config.CHANGE_DOWN) * 100;
    ...
    save: function() {
      ...
      let fixed_up = this.$refs.fixed_up.value;
      let fixed_down = this.$refs.fixed_down.value; 
      ...
      let setting = {
        is_fixed_price_watched: this.is_fixed_open,
        fixed_price_up: fixed_up,
        fixed_price_down: fixed_down,
        period_up: periodup,
